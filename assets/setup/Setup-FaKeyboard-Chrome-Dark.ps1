...
# ====================================================================
#  بخش اصلی — همه‌چیز بدون لاگ‌اوت فورس می‌شود
# ====================================================================

$ErrorActionPreference = 'Continue'

# --- تابع فورس‌رفرش: اعلام تغییر تنظیمات به همه برنامه‌ها ---
Add-Type @"
using System;
using System.Runtime.InteropServices;
...
function Send-SettingChange([string]$Area) {
    ...
    } catch { Write-Warning "SettingChange($Area) failed: $_" }
}

try {

    Write-Host ""
    Write-Host "=== 1/4: Adding Persian (Farsi) keyboard ===" -ForegroundColor Cyan
    ...
    # فورس‌رفرش لیست زبان + اجرای سرویس زبان (بدون نیاز به لاگ‌اوت)
    Send-SettingChange 'intl'
    Start-Process "$env:SystemRoot\System32\ctfmon.exe" -ErrorAction SilentlyContinue

    Write-Host ""
    Write-Host "=== 2/4: Setting Alt+Shift for language switching ===" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "=== 3/4: Setting Google Chrome as default browser ===" -ForegroundColor Cyan

    # --- 3) کروم به عنوان مرورگر پیش‌فرض (متد UserChoice با هش معتبر) ---
    Set-PTA ChromeHTML http
    Set-PTA ChromeHTML https
    Set-FTA ChromeHTML .htm
    Set-FTA ChromeHTML .html
    Send-SettingChange ""
    Write-Host "  Chrome is now the default browser (http/https/htm/html)." -ForegroundColor Green

    Write-Host ""
    Write-Host "=== 4/4: Applying Windows Dark Theme ===" -ForegroundColor Cyan
    ...
    Write-Host " ALL DONE! (no logout needed)" -ForegroundColor Green
    Write-Host " Alt+Shift = تعویض زبان EN/FA" -ForegroundColor Green
    Write-Host "============================================" -ForegroundColor Green
